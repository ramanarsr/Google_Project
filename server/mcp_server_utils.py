...
def load_documents_from_folder(folder_path=os.getenv("DOCUMENT_PATH", "/app/server/data")):
    docs, titles = [], []
...
def summarize_chunks_bullets(chunks: List[str], max_bullets=10, client: Groq = None):
    if not chunks:
        return []
    if client is None:
...
def build_mcp_prompt(query: str, retrieved: List[str], bullets: List[str], memory_text: str = "", max_chars=2500):
    ...
    file_name=r"C:\Users\raman\OneDrive\Desktop\text.txt"
